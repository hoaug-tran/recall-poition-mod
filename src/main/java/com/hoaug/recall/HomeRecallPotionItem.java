...
public class HomeRecallPotionItem extends Item {
    public HomeRecallPotionItem(Settings settings) {
        super(settings);
    }
    ...
    public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
        ...
        RecallState state = RecallState.getServerState(player.getServer());
        GlobalPos homePos = state.getHome(player.getUuid());

        if (homePos == null) {
            player.sendMessage(
                    Text.translatable("message.recall_potion.home_fail").formatted(Formatting.RED),
                    true);
            world.playSound(
                    null,
                    player.getBlockPos(),
        ...
        ServerWorld targetWorld = player.getServer().getWorld(homePos.dimension());
        if (targetWorld == null) {
            player.sendMessage(
                    Text.translatable("message.recall_potion.home_fail").formatted(Formatting.RED),
                    true);
        ...
        BlockPos dest = findSafeDestination(targetWorld, homePos.pos());
        ...
        targetWorld.spawnParticles(ParticleTypes.REVERSE_PORTAL, dest.getX() + 0.5D, dest.getY() + 1.0D,
                dest.getZ() + 0.5D, 50, 0.5D, 1.0D, 0.5D, 0.1D);

        player.sendMessage(
                Text.translatable("message.recall_potion.home_success").formatted(Formatting.GREEN),
                true);
    ...
    public void appendTooltip(
            ...
            TooltipType type) {
        textConsumer.accept(
                Text.translatable("tooltip.recall_potion.home")
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        ...
        if (remainingUseTicks == 39 && world instanceof ServerWorld serverWorld) {
            serverWorld.playSound(null, player.getBlockPos(), SoundEvents.BLOCK_PORTAL_TRIGGER, SoundCategory.PLAYERS,
                    0.3f, 2.0f);
        }
